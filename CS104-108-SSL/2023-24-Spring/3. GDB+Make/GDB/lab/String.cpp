...
public:
    ...
    MyString operator+(const MyString& other) const {
        ...
        size_t p = 0;
        // copy upto and excluding '\0'; when loop terminates p = length
        for (; p < length; p++) newStr[p] = data[p];
        for (; p < newStr.length; p++) newStr[p] = other.data[p - length];
    ...
    MyString& operator+=(const MyString& other) {
        ...
        size_t p = 0;
        // copy upto and excluding '\0'; when loop terminates p = length
        for (; p < length; p++) newData[p] = data[p];
        for (; p < length + other.length; p++) newData[p] = other.data[p - length];
        newData[p] = '\0';
        delete[] data;
        data = newData;
        length += other.length;
    ...
    char& operator[](size_t index) {
        // NOT SURE IF index == length SHOULD BE PERMITTED???
    ...
    friend std::istream& operator>>(std::istream& is,
                                    MyString& str) {  // change this : remove reference
